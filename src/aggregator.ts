...
import { KV_MERGED_CONFIG, KV_MERGED_CONFIG_FULL, KV_SOURCE_URLS, KV_LAST_UPDATE, KV_MANUAL_SOURCES, KV_MACCMS_SOURCES, KV_LIVE_SOURCES, KV_LIVE_MERGED_DATA, KV_BLACKLIST, KV_INLINE_PREFIX, KV_NAME_TRANSFORM, KV_SOURCE_HEALTH, KV_SPEED_TEST_ENABLED, KV_EDGE_PROXIES, KV_SEARCH_QUOTA_REPORT, KV_CHANNEL_MERGED_TREE, KV_AGG_LOGS, AGG_LOGS_MAX, KV_SITE_SNAPSHOT, KV_DEDUP_CONFIG, KV_LIVE_DISABLED, KV_LIVE_MERGE_MODE, KV_IGNORE_AGGREGATED_LIVES, BASE_URL_PLACEHOLDER, KV_SITE_HEALTH_MAP, KV_SITE_PROBE_DEPTH, KV_SITE_AUTO_CLEAN, KV_SOURCE_MAP, KV_SOURCE_URL_BLACKLIST, KV_LOCAL_TOKEN_ENABLED } from './core/config';
...
async function _runAggregation(storage: Storage, config: AppConfig, startTime: number): Promise<void> {
  ...
  if (merged.sites && merged.sites.length > 0) {
    ...
    merged.sites = normalizeConfigCenterSites(merged.sites, localTokenEnabled);
...
function normalizeConfigCenterSites(sites: TVBoxSite[], localTokenEnabled: boolean): TVBoxSite[] {
  ...
  if (localTokenEnabled) {
    const localConfigCenter: TVBoxSite = {
      key: 'csp_Config_Local',
      name: '配置┃本地中心 「176111」',
      type: 3,
      api: 'csp_Config',
      searchable: 0,
    ...
    // 统一插到第二和第三序列
    if (result.length > 0) {
      result.splice(1, 0, localConfigCenter, cloudConfigCenter);
    } else {
      result.push(localConfigCenter, cloudConfigCenter);
    }
  } else {
